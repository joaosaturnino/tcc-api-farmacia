...
module.exports = {
  async listarMedicamentos(request, response) {
    try {
      const sql = `SELECT med_id, med_nome, med_dosagem, med_quantidade, 
                  forma_id, descricao, lab_id, med_img, tipo_id FROM medicamento;`;
  ...
  async cadastrarMedicamentos(request, response) {
    try {
      const { med_nome, med_dosagem, med_quantidade, forma_id, descricao, lab_id, med_img, tipo_id } = request.body;
      const sql = `INSERT INTO medicamento (med_nome, med_dosagem, med_quantidade, 
                  forma_id, descricao, lab_id, med_img, tipo_id) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?);`;
      const values = [med_nome, med_dosagem, med_quantidade, forma_id, descricao, lab_id, med_img, tipo_id];
  ...
  async editarMedicamentos(request, response) {
    try {
      ...
      const sql = `UPDATE medicamento SET med_nome = ?, med_dosagem = ?, med_quantidade = ?, 
                  forma_id = ?, descricao = ?, lab_id = ?, med_img = ?, tipo_id = ? 
                  WHERE med_id = ?;`;
      const values = [med_nome, med_dosagem, med_quantidade, forma_id, descricao, lab_id, med_img, tipo_id, med_id];
      const [rows] = await db.query(sql, values);
      return response.status(200).json({
        sucesso: true,
